...
class bank_reconciliation_xls(report_xls):
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        print "Data---->>", data
        SheetName = data.journal_id.name
        report_name = SheetName
        ...
        ######Kolom TTD#####
        row+=2
        ws.write_merge(row, row, 3,5,"Checked by :", tabel_ttd)
        ws.write_merge(row, row, 6,7,"Created by :", tabel_ttd)
        ...
        ws.write_merge(row, row, 3,5,data.check_by and data.check_by.name or '#NA', tabel_ttd_under_line)
        ws.write_merge(row, row, 6,7,data.user_id.name or '#NA', tabel_ttd_under_line)
